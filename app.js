...
async function openTeamEditor1(){
  hideAllViews();                 // ya existe en tu app
  $("#teamEditor1View").classList.remove("hidden");

  await loadTeamEditor1();
}

async function loadTeamEditor1(){
  const body = $("#teamEditorBody");
  body.innerHTML = `<tr><td colspan="9" class="muted">Loading...</td></tr>`;

  // 1) Traemos directorio de empleados
  const dir = await fetchJSON(`${CONFIG.BASE_URL}?action=getEmployeesDirectory`);
  if(!dir?.ok){
    body.innerHTML = `<tr><td colspan="9">No employees found</td></tr>`;
    return;
  }

  const employees = dir.employees || [];

  // 2) Traemos semana actual (usamos getSmartSchedule sin email => global)
  const week = await fetchJSON(`${CONFIG.BASE_URL}?action=getSmartSchedule`);
  const weekLabel = week?.weekLabel || week?.weekRange || "";
  $("#teamEditorWeekLabel").textContent = weekLabel ? `Week: ${weekLabel}` : "";

  const weekdays = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"];

  // 3) Render tabla
  body.innerHTML = employees.map(emp=>{
    const active = emp.active !== false; // default true
    const dotClass = active ? "emp-dot" : "emp-dot inactive";

    // schedule global trae mapa por email, si existe:
    const empWeek = week?.byEmail?.[emp.email] || {};

    const tds = weekdays.map((d,i)=>{
      // guardamos dayIndex (0=Mon..6=Sun)
      const val = empWeek?.[i] || ""; 
      return `
        <td>
          <input class="shift-input"
                 data-email="${emp.email}"
                 data-day="${i}"
                 value="${escapeHTML(val)}"
                 placeholder="Off / 9am-5pm"/>
        </td>
      `;
    }).join("");

    return `
      <tr data-email="${emp.email}" data-name="${escapeHTML(emp.name||"")}" data-role="${escapeHTML(emp.role||"")}">
        <td>
          <div class="emp-name-wrap">
            <span class="emp-open" onclick="openTeamViewFromEditor('${emp.email}')">
              ${escapeHTML(emp.name||emp.email)}
            </span>
            <span class="${dotClass}" title="${active?'Active':'Inactive'}"></span>
          </div>
        </td>
  ...
  // 4) Listener para guardar cambios con debounce
  setupTeamEditorAutosave();
  setupTeamEditorButtons();
}

function setupTeamEditorAutosave(){
  const inputs = $$("#teamEditorBody .shift-input");
  const debounced = debounce(async (inp)=>{
    const email = inp.dataset.email;
    const dayIndex = Number(inp.dataset.day);
    const value = inp.value.trim();

    // updateShift endpoint ya existe en tu backend R1
    const r = await fetchJSON(`${CONFIG.BASE_URL}?action=updateShift&email=${encodeURIComponent(email)}&day=${dayIndex}&value=${encodeURIComponent(value)}`);
    if(!r?.ok){
      toast("❌ Error saving shift");
      return;
    }
    toast("✅ Saved");
  }, 500);
  ...
  inputs.forEach(inp=>{
    inp.addEventListener("input", ()=>debounced(inp));
  });
}

function setupTeamEditorButtons(){
  $("#btnSendTodayGlobal").onclick = async ()=>{
    btnBusy("#btnSendTodayGlobal", true);
    try{
      const r = await fetchJSON(`${CONFIG.BASE_URL}?action=sendtoday&global=1`);
      toast(r?.ok ? "✅ Sent Today (Global)" : "❌ Error Send Today");
    }finally{
      btnBusy("#btnSendTodayGlobal", false);
    }
  };

  $("#btnSendTomorrowGlobal").onclick = async ()=>{
    btnBusy("#btnSendTomorrowGlobal", true);
    try{
      const r = await fetchJSON(`${CONFIG.BASE_URL}?action=sendtomorrow&global=1`);
      toast(r?.ok ? "✅ Sent Tomorrow (Global)" : "❌ Error Send Tomorrow");
    }finally{
      btnBusy("#btnSendTomorrowGlobal", false);
    }
  };

  $("#btnCreateNewWeek").onclick = async ()=>{
    if(!confirm("Create a new week tab from Master?")) return;
    btnBusy("#btnCreateNewWeek", true);
    try{
      const r = await fetchJSON(`${CONFIG.BASE_URL}?action=createNewWeek`);
      toast(r?.ok ? `✅ New week created: ${r.tabName}` : "❌ Error creating week");
      if(r?.ok) await loadTeamEditor1(); // recarga
    }finally{
      btnBusy("#btnCreateNewWeek", false);
    }
  };
}

// abre Team View individual desde la tabla
function openTeamViewFromEditor(email){
  // tu team view actual abre por email,
  // asumo que tienes una función tipo openTeamView(email)
  if(typeof openTeamView === "function") return openTeamView(email);

  // fallback: navegar a team view y cargar schedule
  openTeamViewModalByEmail(email); // si ya la tienes
}
