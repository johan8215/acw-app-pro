...
/* ============================================================
   ACW-App v5.6.3 Turbo — Clean Core R2 (Nov 2025)
   JAG15 & Sky  —  TTL cache + de-dupe + Team View seguro
   ============================================================ */

const CONFIG = window.CONFIG || {};
CONFIG.VERSION = CONFIG.VERSION || "v5.6.3 Turbo — Clean Core R2";
...
const setVisible=(el,show)=>{ if(!el) return; el.style.display = show? "" : "none"; };
const safeText=(el,txt)=>{ if(el) el.textContent = txt; };
const isManagerRole=(r)=>["manager","supervisor"].includes(String(r||"").toLowerCase());
const cssEsc=(s)=>{ try{ return CSS.escape(s); }catch{ return String(s).replace(/[^a-zA-Z0-9_\-]/g,"_"); } };
...
function toast(msg,type="info"){
  ...
  t.style.background = type==="success" ? "linear-gradient(135deg,#00c851,#007e33)" :
                    type==="error" ? "linear-gradient(135deg,#ff4444,#cc0000)" :
                                     "linear-gradient(135deg,#007bff,#33a0ff)";
  Object.assign(t.style,{color:"#fff",padding:"10px 16px",marginTop:"8px",borderRadius:"8px",
    fontWeight:"600",boxShadow:"0 6px 14px rgba(0,0,0,.25)",opacity:"0",transform:"translateY(-10px)",
    transition:"all .35s ease",maxWidth:"78vw"});
  $("#toastContainer").appendChild(t);
  requestAnimationFrame(()=>{ t.style.opacity="1"; t.style.transform="translateY(0)"; });
  setTimeout(()=>{ t.style.opacity="0"; t.style.transform="translateY(-10px)"; setTimeout(()=>t.remove(),360); }, 2600);
}

/* ---------- Today key (auto a medianoche) ---------- */
const Today = (()=>{
  let key = new Date().toLocaleString("en-US",{weekday:"short"}).slice(0,3).toLowerCase();
  const now=new Date(), next=new Date(now); next.setHours(24,0,0,0);
  setTimeout(()=>{ key = new Date().toLocaleString("en-US",{weekday:"short"}).slice(0,3).toLowerCase(); }, (next-now)+80);
  return { get key(){return key;} };
})();

/* ---------- Cache de red con TTL + de-dupe ---------- */
const Net = (()=> {
  const m=new Map(); // key -> {expires,value} o {inflight:Promise}
  return {
    get(k){ const it=m.get(k); if(!it) return null; if(it.value && it.expires>Date.now()) return it.value; if(it.inflight) return it.inflight; m.delete(k); return null; },
    set(k,v,ttl){ m.set(k,{value:v,expires:Date.now()+ttl}); return v; },
    inflight(k,p){ m.set(k,{inflight:p,expires:0}); },
    clearInflight(k){ const it=m.get(k); if(it?.inflight) m.delete(k); }
  };
})();
async function fetchJSON(url,{ttl=0,signal}={}){
  if(ttl>0){ const c=Net.get(url); if(c) return c; }
  const p = fetch(url,{cache:"no-store",signal}).then(r=>r.json());
  if(ttl>0) Net.inflight(url,p);
  try{ const j=await p; if(ttl>0) Net.set(url,j,ttl); return j; } finally { if(ttl>0) Net.clearInflight(url); }
}
...
function deriveAliasCandidates(full){
  full=(full||"").replace(/\s+/g," ").trim();
  const JOINERS=new Set(["DE","DEL","LA","DE LA","DELA","DE LAS","DE LOS","DA","DOS","VON","VAN","DI","DAL"]);
  let parts=full.split(" ").filter(Boolean); if(parts.length<2) return [];
  let last=parts[parts.length-1], prev=(parts[parts.length-2]||"");
  if(JOINERS.has(prev.toUpperCase())){ last=`${prev} ${last}`; parts=parts.slice(0,-2); } else { parts=parts.slice(0,-1); }
  const LAST=last.toUpperCase().replace(/[^A-ZÁÉÍÓÚÜÑ ]/g,"").trim();
  const initials = parts.map(p=>p.replace(/[^A-Za-zÁÉÍÓÚÜÑ]/g,"").charAt(0).toUpperCase()).filter(Boolean);
  const F=initials[0]||"", FI=(initials[0]||"")+(initials[1]||"");
  const v=new Set([ LAST, F&&`${F}. ${LAST}`, F&&`${F} ${LAST}`, FI&&`${FI}. ${LAST}`, FI&&`${FI} ${LAST}`,
                    initials[1]?`${initials[0]}.${initials[1]}. ${LAST}`:null, initials[1]?`${initials[0]}. ${initials[1]}. ${LAST}`:null ].filter(Boolean));
  // + variantes con coma y sin puntos
  const plus = new Set([
    LAST&&F?`${LAST}, ${F}.`:null, LAST&&F?`${LAST}, ${F}`:null,
    LAST&&FI?`${LAST}, ${FI}.`:null, LAST&&FI?`${LAST}, ${FI}`:null
  ].filter(Boolean));
  const nodot = Array.from(v).map(s=>s.replace(/\./g,"").replace(/\s{2,}/g," ").trim());
  return Array.from(new Set([...v, ...plus, ...nodot]));
}
...
const API = {
  ...
  async getDirectory(controller){
    const base = CONFIG.BASE_URL;
    const u = `${base}?action=getEmployeesDirectory`;
    const j = await fetchJSON(u,{ttl:this.dirTTL, signal:controller?.signal});
    // normaliza
    const list = j?.directory || j?.employees || j?.rows || [];
    return { ok: Array.isArray(list), directory: list };
  },
  ...
  async resolveAlias({email, phone}={}, controller){
    ...
    const norm=v=>(v||"").toString().trim();
    const nPhone=v=>norm(v).replace(/\D/g,"");
    const rec = (d.directory||[]).find(x =>
      (email && norm(x.email).toLowerCase()===norm(email).toLowerCase()) ||
      (phone && nPhone(x.phone) && nPhone(x.phone)===nPhone(phone))
    );
    if(!rec){ const r={alias:"", candidates:[], matched:false}; this._aliasCache.set(key,r); return r; }

    const full = norm(rec.name||rec.employee||rec.fullname||"");
    const primary = deriveAliasFromFullName(full);
    const candidates = Array.from(new Set([primary, ...deriveAliasCandidates(full)].filter(Boolean)));
    ...
    const check = async (a)=>{
      for(const action of ["getSmartSchedule","getScheduleByAlias","getSchedule"]){
        try{
          const r = await fetchJSON(`${base}?action=${action}&alias=${encodeURIComponent(a)}&offset=0`, {ttl:this.schedTTL0, signal});
          const days = r?.days || r?.week?.days || r?.schedule || [];
          if(Array.isArray(days) && days.length) return true;
        }catch{}
      }
    ...
    let matched=null;
    for(const a of candidates){ if(await check(a)){ matched=a; break; } }
    const out = { alias: matched||primary, candidates, matched: !!matched };
    this._aliasCache.set(key,out);
    return out;
  },
  ...
  async getSchedule(identifier, offset=0, controller){
    ...
    const ttl = offset===0 ? this.schedTTL0 : this.schedTTLOld;
    const signal=controller?.signal;
    const fetchN = async (u)=>{
      try{
        const raw = await fetchJSON(u,{ttl,signal});
        // normalize
        let daysArr = raw?.days || raw?.week?.days || raw?.schedule || raw?.rows;
        if(!Array.isArray(daysArr)){
          const keys=["mon","tue","wed","thu","fri","sat","sun","Mon","Tue","Wed","Thu","Fri","Sat","Sun"];
          if(keys.some(k=>raw&&k in raw)) daysArr = keys.filter(k=>k in raw).map(k=>({name:k,shift:raw[k]}));
        }
        const days = Array.isArray(daysArr) ? daysArr.map(x=>{
          const name=x?.name||x?.day||"", shift=(x?.shift ?? x?.text ?? x ?? "");
          const hours=Number(x?.hours ?? 0) || parseHours(shift);
          return { name, shift, hours };
        }) : [];
        const total = (typeof raw?.total === "number") ? raw.total : days.reduce((s,r)=>s+(Number(r.hours)||0),0);
        return { ok: days.length>0, days, total, rowAlias: raw?.rowAlias||raw?.alias||null, weekLabel: raw?.weekLabel||raw?.label||"" };
      }catch{ return { ok:false, days:[], total:0 }; }
    };
    ...
    // por alias
    let aliasInfo=null; try{ aliasInfo=await this.resolveAlias({email:identifier}, controller); }catch{}
    const probe = [];
    if(aliasInfo?.candidates) probe.push(...aliasInfo.candidates);
    if(aliasInfo?.alias)      probe.push(aliasInfo.alias);
    const unique = Array.from(new Set(probe));
    for(const a of unique){
      for(const act of ["getSmartSchedule","getScheduleByAlias","getSchedule"]){
        res = await fetchN(`${base}?action=${act}&alias=${encodeURIComponent(a)}&offset=${offset}`);
        if(res.ok) return res;
      }
    }
    return res;
...
async function loginUser(){
  const email=$("#email")?.value.trim();
  const password=$("#password")?.value.trim();
  const diag=$("#diag"); const btn=$("#signInBtn")||$("#login button");
  if(!email || !password){ safeText(diag,"Please enter your email and password."); return; }
  if(!CONFIG.BASE_URL){ safeText(diag,"⚠️ Missing BASE_URL in CONFIG."); return; }

  try{
    if(btn){ btn.disabled=true; btn.textContent="⏳ Loading your shift…"; }
    safeText(diag,"Connecting to Allston Car Wash servers ☀️");
    const u = `${CONFIG.BASE_URL}?action=login&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`;
    const j = await fetchJSON(u,{ttl:0});
    if(!j?.ok) throw new Error(j?.error || "Invalid email or password.");
    currentUser=j; localStorage.setItem("acwUser", JSON.stringify(j));
    safeText(diag,"✅ Welcome, "+j.name+"!");
    await showWelcome(j.name, j.role);
    await loadSchedule(j.email);
  }catch(e){
    safeText(diag,"❌ "+(e.message||"Login error"));
  }finally{
...
async function showWelcome(name, role){
  ...
  if(isManagerRole(role)) ensureTeamButton();

  // teléfono propio
  try{
    const d = await API.getDirectory();
    const self=(d.directory||[]).find(e=>(e.email||"").toLowerCase()===(currentUser?.email||"").toLowerCase());
    if(self?.phone){
      $(".user-phone")?.remove();
      $("#welcomeName")?.insertAdjacentHTML("afterend", `<p class="user-phone">📞 <a href="tel:${self.phone}" style="color:#0078ff;font-weight:600;text-decoration:none;">${self.phone}</a></p>`);
    }
  }catch{}
}
...
async function loadSchedule(email){
  const out=$("#schedule");
  out.innerHTML = `<p style="color:#007bff;font-weight:500;">Loading your shift...</p>`;
  try{
    const d = await API.getSchedule(email, 0);
    const days = d?.days || [];
    if(!days.length){ out.innerHTML=`<p style="color:#c00;">No schedule found for this week.</p>`; return; }

    const todayKey=Today.key;
    let html = `<table><thead><tr><th>Day</th><th>Shift</th><th style="text-align:right">Hours</th></tr></thead><tbody>`;
    days.forEach(day=>{
      const isToday = todayKey === String(day.name||"").slice(0,3).toLowerCase();
      html += `<tr class="${isToday?"today":""}">
        <td>${day.name||""}</td>
        <td>${String(day.shift||"-").replace(/\s-\s/g,'&nbsp;–&nbsp;')}</td>
        <td style="text-align:right">${Number(day.hours||0).toFixed(1)}</td>
      </tr>`;
    });
    html += `</tbody></table><p class="total">Total Hours: <b>${Number(d.total||0).toFixed(1)}</b></p>`;
    out.innerHTML = html;

    // LIVE
    clearInterval(__liveTimer);
    const today=days.find(x=> x.name.slice(0,3).toLowerCase()===todayKey);
    if(!today || !today.shift || /off/i.test(today.shift)) return;
    const row=[...out.querySelectorAll("tbody tr")].find(r=> r.cells?.[0]?.textContent.slice(0,3).toLowerCase()===todayKey);
    if(!row) return;

    const liveP = document.createElement("p"); liveP.className="live-hours"; liveP.style.marginTop="6px";
    out.appendChild(liveP);

    function parseTime(str){
      const m=String(str||"").trim().match(/^(\d{1,2})(?::(\d{1,2}))?\s*(am|pm)?$/i); if(!m) return null;
      let h=+m[1], min=+(m[2]||0), s=(m[3]||"").toLowerCase(); if(s==="pm"&&h<12)h+=12; if(s==="am"&&h===12)h=0;
      const d=new Date(); d.setHours(h,min,0,0); return d;
    }
    function updateTotalDisplay(val,active){
      const t=$(".total"); if(!t||isNaN(val)) return;
      t.innerHTML = `<span style="color:${active?"#33a0ff":"#e60000"}">⚪ Total Hours: <b>${val.toFixed(1)}</b></span>`;
    }

    const shift=today.shift.trim();
    const baseTotal = Number((d.total||0)) - Number(today.hours||0);

    if(shift.endsWith(".")){
      const start = parseTime(shift.replace(/\.$/,"").trim()); if(!start) return;
      const tick=()=>{
        const diff=Math.max(0,(Date.now()-start.getTime())/36e5);
        row.cells[2].innerHTML=`⏱️ ${diff.toFixed(1)}h`;
        row.cells[2].style.color="#33a0ff"; row.cells[2].style.fontWeight="600";
        liveP.innerHTML=`⏱️ <b style="color:#33a0ff">${diff.toFixed(1)}h</b>`;
        updateTotalDisplay(baseTotal+diff,true);
      };
      tick(); __liveTimer=setInterval(tick,60000);
    }else{
      const p=shift.split("-"); if(p.length===2){
        const a=parseTime(p[0].trim()), b=parseTime(p[1].trim()); if(!a||!b) return;
        const diff=Math.max(0,(b-a)/36e5);
        row.cells[2].textContent=`${diff.toFixed(1)}h`; row.cells[2].style.color="#999";
        liveP.textContent=""; updateTotalDisplay(baseTotal+diff,false);
      }
    }
  }catch(e){
    out.innerHTML = `<p style="color:#c00;">Error loading schedule.</p>`;
    console.warn(e);
  }
}

/* ---------- Settings (abre siempre) ---------- */
function openSettings(){
  // quita overlays que tapen
  $("#acwhOverlay")?.remove();
  $("#directoryWrapper")?.remove();
  const m=$("#settingsModal");
  if(!m){ console.warn("Settings modal not found"); return; }
  m.style.display="flex"; m.style.alignItems="center"; m.style.justifyContent="center"; m.style.zIndex=12000;
  requestAnimationFrame(()=> m.classList.add("show"));
  const onClick=(e)=>{ if(e.target===m) closeSettings(); };
  const onKey =(e)=>{ if(e.key==="Escape") closeSettings(); };
  m.addEventListener("click", onClick, { once:true });
  document.addEventListener("keydown", onKey, { once:true });
}
...
function refreshApp(){
  try{ if("caches" in window) caches.keys().then(keys=>keys.forEach(k=>caches.delete(k))); }catch{}
  toast("⏳ Updating…","info"); setTimeout(()=>location.reload(), 900);
}
function logoutUser(){
  localStorage.removeItem("acwUser"); toast("👋 Logged out","info"); setTimeout(()=>location.reload(), 400);
}
...
async function submitChangePassword(){
  ...
  if(newPass.length<6) return safeText(diag,"⚠️ Password must be at least 6 characters.");
  try{
    const email=currentUser?.email; if(!email) throw new Error("Session expired. Please log in again.");
    safeText(diag,"⏳ Updating password...");
    const u=`${CONFIG.BASE_URL}?action=changePassword&email=${encodeURIComponent(email)}&oldPass=${encodeURIComponent(oldPass)}&newPass=${encodeURIComponent(newPass)}`;
    const j=await fetchJSON(u,{ttl:0});
    if(j?.ok){ safeText(diag,"✅ Password updated successfully!"); toast("✅ Password updated","success");
      setTimeout(()=>{ closeSettings(); $("#oldPass").value=$("#newPass").value=$("#confirmPass").value=""; }, 900);
    }else{ safeText(diag,"❌ "+(j?.error||"Invalid current password.")); }
  }catch(err){ safeText(diag,"⚠️ "+(err.message||err)); }
}

/* ---------- Team View (sin atributos onclick) ---------- */
const TEAM_PAGE_SIZE=8; let __teamList=[], __teamPage=0, __tvController=null, __tvInterval=null;
