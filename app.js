...
// === ACW PATCH v5.6.3 — getSchedule robusto (añadir debajo del cierre del API) ===
(function patchGetSchedule(){
  ...
  function normDay(x){
    if (typeof x === "string") return { name:"", shift:x, hours:parseHours2(x) };
    const name  = x?.name || x?.day || x?.dow || "";
    const shift = x?.shift ?? x?.text ?? x?.value ?? "";
    const hours = Number(x?.hours ?? 0) || parseHours2(shift);
    return { name, shift, hours };
  }
