...
async function refreshTeamEditorVisibleRows(controller){
  const wrap = document.getElementById("teamEditorWrapper");
  if (!wrap) return;

  // agarramos filas YA reales
  const rowEls = Array.from(wrap.querySelectorAll('tr[data-email]'));
  ...
  await runLimited(rows, 4, async (info)=>{
    const email = info.email;
    const tr = info.rowEl;

    const d = await API.getSchedule(email, 0, controller);
    if (!d?.ok) return;

    // ✅ actualiza shifts por día
    (d.days || []).forEach(day=>{
      const day3 = String(day.name||"").slice(0,3);
      const cell = tr.querySelector(`[data-day="${day3}"]`);
      if (!cell) return;

      const newShift = String(day.shift||"-").trim();
      if (cell.__lastShift !== newShift){
        cell.__lastShift = newShift;
        cell.textContent = newShift;
      }
    });

    // ✅ total hours correcto PARA ESA FILA
    const tot = tr.querySelector(".te-total");
    if (tot){
      const t = Number(d.total||0).toFixed(1);
      if (tot.__lastTot !== t){
        tot.__lastTot = t;
        tot.textContent = t;
      }
...
/* =========================================================
   HOTFIX v5.6.3 — Alias específico primero (apellidos repetidos)
   Soluciona casos como D. MEJIA / M. MEJIA / F. MEJIA
   Pégalo AL FINAL del JS
   ========================================================= */
(function(){
  if (typeof API==="undefined" || typeof fetchJSON==="undefined") return;

  function aliasVariantsFromName(full){
    if (!full) return [];
    ...
    const first = (parts[0]||"").toUpperCase();
    let last = (parts[parts.length-1]||"").toUpperCase();
    const JOIN = new Set(["DE","DEL","DE LA","DE LOS","DE LAS","DA","VON","VAN","DI","DAL"]);
    const prev = (parts[parts.length-2]||"").toUpperCase();
    if (JOIN.has(prev)) last = `${prev} ${last}`;
    const fi = first[0]||"";
    const NBSP="\u00A0";
    const base = [
      `${fi}. ${last}`, `${fi}.${last}`, `${fi} ${last}`,
      `${fi}${NBSP}.${NBSP}${last}`,
      `${first} ${last}`,
      last
    ];
  ...
  API.getSchedule = async function(identifier, offset=0, controller){
    ...
    // 1) intenta alias específicos primero
    try{
      const dir = await API.getDirectory(controller);
      const list = dir?.directory || dir?.employees || [];
      const rec = list.find(r => (r.email||"").toLowerCase()===email.toLowerCase());
      const aliases = aliasVariantsFromName(rec?.name);

      for (const a of aliases){
        for (const act of ["getSmartSchedule","getScheduleByAlias","getSchedule"]){
          const url = `${base}?action=${act}&alias=${encodeURIComponent(a)}&offset=${offset}`;
          try{
            const raw = await fetchJSON(url,{ttl,signal});
            const n = normalize(raw);
            if (n.ok) return { ...n, raw };
          }catch{}
        }
      }
    }catch{}

    // 2) fallback normal por correo
    return _orig.call(this, email, offset, controller);
  };

  console.log("✅ HOTFIX alias específico primero activo");
})();
